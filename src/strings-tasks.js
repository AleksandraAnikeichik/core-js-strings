...
function formatTime(minutes, seconds) {
  ...
  const returnMinutes = String(minutes).padStart(2, '0');
  const returnSeconds = String(seconds).padStart(2, '0');
  return returnMinutes.concat(`:${returnSeconds}`);
}
...
function countVowels(str) {
  ...
  const numOfLatter = str.match(vowels);
  return numOfLatter ? numOfLatter.length : 0;
}
...
function isPalindrome(str) {
  const strNew = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
...
function extractNameFromTemplate(value) {
  const temp = value.slice(0, value.length - 1).split(' ');
...
function encodeToRot13(str) {
  ...
  const revAlphabet = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const revRow = [];
  str.split(' ').forEach((word) => {
    const revWord = [];
    word.split('').forEach((char) => {
      if (alphabet.includes(char)) {
        revWord.push(revAlphabet.charAt(alphabet.indexOf(char)));
      } else {
        revWord.push(char);
      }
    });
    revRow.push(revWord.join(''));
  });
  return revRow.join(' ');
}
